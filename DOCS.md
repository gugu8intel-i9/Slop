...
- `slop_sir_optimizer.h`: Phase-3 high-performance optimizer for linear folding, algebraic simplification, hash CSE, string fusion, branch folding, jump cleanup, pure DCE, duplicate/proven bounds-check elimination, SEAA arena compression, fixed-point rounds, and parallel-safety classification.
